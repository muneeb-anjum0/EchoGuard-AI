...
from ..core.settings import SAMPLE_RATE
...
def spectrogram_base64(audio: np.ndarray, sample_rate: int = SAMPLE_RATE) -> str:
    audio = audio if audio.size else np.zeros(sample_rate, dtype=np.float32)

    fig, ax = plt.subplots(figsize=SPECTROGRAM_STYLE["figure_size"], dpi=SPECTROGRAM_STYLE["dpi"])
    spectrogram = librosa.amplitude_to_db(
        np.abs(librosa.stft(audio, n_fft=SPECTROGRAM_STYLE["n_fft"], hop_length=SPECTROGRAM_STYLE["hop_length"])),
        ref=np.max,
    )
    img = librosa.display.specshow(
        spectrogram,
        sr=sample_rate,
        hop_length=SPECTROGRAM_STYLE["hop_length"],
        x_axis="time",
        y_axis="log",
        cmap="magma",
        ax=ax,
    ...
    ax.set_ylabel("")
    ax.set_title("")
    ax.tick_params(colors=SPECTROGRAM_STYLE["tick_color"], labelsize=7)
    for spine in ax.spines.values():
        spine.set_visible(False)
    fig.colorbar(img, ax=ax, format="%+2.0f dB")
    fig.patch.set_facecolor(SPECTROGRAM_STYLE["background"])
    ax.set_facecolor(SPECTROGRAM_STYLE["background"])
