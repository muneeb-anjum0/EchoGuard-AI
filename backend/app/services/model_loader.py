...
from ..core.settings import MODEL_ARTIFACT_DIR, MODEL_ROOT
...
def resolve_or_extract_model(artifact_name: str, target_dir: Path) -> Path:
    ...
    artifact_path = MODEL_ARTIFACT_DIR / artifact_name
...
def find_hf_model_dir(root: Path) -> Path | None:
    ...
    for path in [root, *root.rglob("*")]:
        ...
        if REQUIRED_MODEL_FILES.issubset(child_names) and child_names.intersection(WEIGHT_FILES):
            return path
    return None
